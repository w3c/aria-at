...
function findValueByKey(keyMappings, keyToFind) {
...
function findValuesByKeys(keyMappings, keysToFind = []) {
  const result = [];
  ...
  const patternSepWithReplacement = (keyToFind, pattern, replacement) => {
    if (keyToFind.includes(pattern)) {
      ...
      for (const key of keys) {
        const keyResult = findValueByKey(keyMappings, key);
  ...
  const patternSepHandler = keyToFind => {
    ...
    if (keyToFind.includes(' ') && keyToFind.includes('+')) {
      ...
      for (let [index, key] of keys.entries()) {
        const keyToFindResult = findValueByKey(keyMappings, key);
  ...
  for (const keyToFind of keysToFind) {
    if (keyToFind.includes(' ') || keyToFind.includes('+')) {
      ...
      const keyToFindResult = findValueByKey(keyMappings, keyToFind);
...
function parseCommandCSVRowV2({ jawsCommands, nvdaCommands, voCommands }, commandsJson) {
  ...
  function createParsedCommand(command, atTargetInfo) {
    ...
    const foundCommandIndex = commandsParsed.findIndex(
      e => e.target.at.key === atTargetInfo.key && e.target.at.settings === command.settings
    );
  ...
  generateCommandsParsed(jawsCommands, 'jaws');
  generateCommandsParsed(nvdaCommands, 'nvda');
  generateCommandsParsed(voCommands, 'voiceover_macos');

  return commandsParsed;
}

exports.parseCommandCSVRow = parseCommandCSVRow;
exports.parseCommandCSVRowV2 = parseCommandCSVRowV2;
