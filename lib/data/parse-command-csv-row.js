...
function parseCommandCSVRowV2({ jawsCommands, nvdaCommands, voCommands }, commandsJson) {
  ...
  function createParsedCommand(command, atTargetInfo) {
    ...
    if (foundCommandIndex < 0) {
      ...
      commandsParsed[foundCommandIndex].commands.push(...commands, ...assertionExceptions);
    }
  }
  ...
  generateCommandsParsed(jawsCommands.map(sanitizeCommand), 'jaws');
  generateCommandsParsed(nvdaCommands.map(sanitizeCommand), 'nvda');
  generateCommandsParsed(voCommands.map(sanitizeCommand), 'voiceover_macos');
