...
const processTestDirectory = async ({ directory, args = {} }) => {
  ...
  const commandsJson = JSON.parse(commandsRecord.text);
  ...
  const atCommandsCsvFilePaths = getAtCommandsCsvFilePaths(
    testPlanDirectory,
   ...
   * @return {array[string]}
   */
  function getAtCommandsCsvFilePaths(root, fileNames = []) {
    let filePaths = [];
