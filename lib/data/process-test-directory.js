...
`;
  ...
  function addCommandError({ testId }, key) {
    errorCount += 1;
    errors += `[Command]: The key reference "${key}" found in "${directory}/data/*-commands.csv" for "test id ${testId}" is invalid. Command may not be defined in "tests/commands.json".\n`;
  }
  ...
  const supportQueryables = {
    ...
    atGroups: Queryable.from('atGroup', supportParsed.atGroups),
