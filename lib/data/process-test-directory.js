...
`;
  ...
  console.log('testsParsed', JSON.stringify(testsParsed, null, 2));
  ...
  const keyDefs = flattenObject(commands);
  ...
  console.log('commandsValidated', JSON.stringify(commandsValidated, null, 2));
  ...
  testsParsed.forEach(function (testParsed) {
    try {
      ...
      let sequenceNumber = commandsValidated.find(
        c =>
          testParsed.testId === c.testId &&
          testParsed.target.ats[0].key === c.target.at.key &&
          testParsed.target.ats[0].settings === c.target.at.settings
...
function validateCommand(
  ...
  console.log('commandParsed', JSON.stringify(commandParsed, null, 2));
  return {
    ...commandParsed,
    target: {
    ...
    commands: commandParsed.commands.map(({ id, keypresses, ...rest }) => {
    ...
    assertionExceptions: commandParsed.assertionExceptions.map(each => {
...
function validateTest(testParsed, data, { addTestError = () => {} } = {}) {
  ...
  const assertions = testParsed.assertions.map(assertion => {
    if (
      ...
      addTestError(
        `Level value must be 1, 2 or 3, value found was "${assertion.priority}" for assertion "${assertion.assertionStatement}" (NOTE: level 3 defined for this assertion).`
      );
      return {
        ...assertion,
        priority: 3,
