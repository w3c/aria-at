...
`;
  ...
  // [ [ jaws ], [ nvda ], [ vo ]]
  const collectedTestsGroupedByAt = allAtKeys.map(atKey =>
    _testsCollected.filter(collected => collected.target.at.key === atKey)
  );

  for (const collectedTestAtGroup of collectedTestsGroupedByAt) {
    let common = collectedTestAtGroup[0];
    ...
    if (collectedTestAtGroup.length > 1) {
      for (const additional of collectedTestAtGroup.slice(1)) {
  ...
  const files = [
    ...
    // TODO: If there is a need to individually view jaws-pcCursor or nvda-browseMode
