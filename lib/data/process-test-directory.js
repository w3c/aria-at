...
const processTestDirectory = async ({ directory, args = {} }) => {
  ...
  const validModes = ['readingMode', 'interactionMode', 'NA'].concat(atSettings);
  ...
  function createTestFile(
    ...
    let modes = appliesTo.map(getModeValue).join('_');
...
`;
  ...
  // );
  // console.debug('commandsParsed', commandsParsed)
  ...
  const testsCollected = testsValidated.flatMap(test => {
    return test.target.at.map(({ key, settings }) =>
      collectTestData({
        ...
        key: keyQueryable,
        modeInstructionTemplate: MODE_INSTRUCTION_TEMPLATES_QUERYABLE(support),
      })
    );
  });
  ...
  testsParsed.forEach(function (testParsed) {
    try {
      ...
      let sequenceNumber = commandsParsed.find(
        c =>
          testParsed.testId === c.testId &&
          testParsed.target.at[0].key === c.target.at.key &&
          testParsed.target.at[0].settings === c.target.at.settings
      ).commands[0].presentationNumber;
