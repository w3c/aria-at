...
const { parseCommandCSVRowV2 } = require('./parse-command-csv-row');
const {
...
const processTestDirectory = async ({ directory, args = {} }) => {
  ...
  function validateCommandsKeys(row) {
    ...
    if (row.settings && !idFormat.test(row.settings))
  ...
  const titleFormat = /^[A-Z][A-Za-z]+( [A-Za-z]+){2,}[^.]$/;
  function validateTestsKeys(row) {
    ...
    if (
      ...
      throw new Error(
        'Missing one of required testId, title, presentationNumber, setupScript, instructions, assertions'
      );
    }
    if (!idFormat.test(row.testId)) throw new Error('testId does not match the expected format');
  ...
  const assertionStatementFormat = /^[A-Z][A-Za-z' ]*[A-Za-z][A-Za-z' ]*[A-Za-z']$/;
  const assertionPhraseFormat = /^[a-z][A-Za-z' ]*[A-Za-z][A-Za-z' ]*[A-Za-z']$/;
  ...
  const setupScriptDescription = /^[a-z][A-Za-z' ]*[A-Za-z][A-Za-z' ]*[A-Za-z']$/;
  function validateScriptsKeys(row) {
    ...
    if (!setupScriptDescription.test(row.setupScriptDescription))
  ...
  const [
    ...
    readCSVFile('data/jaws-commands.csv', validateCommandsKeys),
    readCSVFile('data/nvda-commands.csv', validateCommandsKeys),
    readCSVFile('data/voiceover_macos-commands.csv', validateCommandsKeys),
  ]);
