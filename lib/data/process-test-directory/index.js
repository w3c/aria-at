...
const processTestDirectory = async config => {
  ...
  const knownSettings = ['readingMode', 'interactionMode', 'defaultMode'].concat(atSettings);
  ...
  function createTestFile(
    ...
    let modes = test.target.ats.map(getSettingsValues).join('_');
  ...
  const testLookups = {
    ...
    mode: Queryable.from('mode', knownSettings),
