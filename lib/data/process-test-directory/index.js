...
// Regex Formats - Common regex patterns found in *.csvs
...
const validTestsKeys = /^(?:testId|title|presentationNumber|setupScript|instructions|assertions)$/;
const titleFormat = /^[A-Z]([A-Za-z-',\s]){2,}[^.]$/;
const assertionsFormat = /[A-Za-z0-9-]+\s*$/;
const invalidPriorityFormat = /[4-9]:/;

const validAssertionsKeys = /^(?:assertionId|priority|assertionStatement|assertionPhrase|refIds)$/;
...
/**
 * @param {object} row
 * @param {string} row.testId
 * @param {string} row.title
 * @param {string} row.presentationNumber
 * @param {string} [row.setupScript]
 * @param {string} row.instructions
 * @param {string} row.assertions
 */
function validateTestsKeys(row) {
  ...
  if (
    row.assertions &&
    (!assertionsFormat.test(row.assertions) || invalidPriorityFormat.test(row.assertions))
  )
...
exports.validateTestsKeys = validateTestsKeys;

/**
 * @param {object} row
 * @param {string} row.assertionId
 * @param {string} row.priority
 * @param {string} row.assertionStatement
 * @param {string} row.assertionPhrase
 * @param {string} [row.refIds]
 */
...
exports.validateAssertionsKeys = validateAssertionsKeys;
...
const processTestDirectory = async config => {
  ...
  let buildTestsDirectory = path.join(buildDirectory, 'tests');
  ...
  if (subfolder) buildTestsDirectory = path.join(buildTestsDirectory, subfolder);
  ...
  if (!someAtCommandsCsvExist) {
    ...
    log.error(
      `No *-commands.csv files found for ${testPlanPath}. Please create at least one of the following: ${missingAtCommandsCsvFiles.join(
        ', '
      )}`
    );
  }

  // TODO: extract and export remaining validators into separate functions for easier unit testing
  ...
  const testPlanPath = subfolder
    ...
    : `tests/${path.basename(testPlanBuildDirectory)}`;
  let newTestPlan = newBuild.find(testPlanPath);

  // If the path doesn't exist (e.g., when subfolder is used), ensure the directory structure exists
  if (!newTestPlan.record) {
    const testsDir = newBuild.find('tests');
    if (!testsDir.record) {
      log.error('Tests directory not found at /build/tests/');
      return;
    }

    if (subfolder) {
      const subfolderDir = testsDir.find(subfolder);
      if (!subfolderDir.record) {
        testsDir.add(subfolder, { entries: [] });
      }

      // Create test plan directory in subfolder
      const subfolderDirUpdated = newBuild.find(`tests/${subfolder}`);
      subfolderDirUpdated.add(path.basename(testPlanBuildDirectory), { entries: [] });
    } else {
      // Test plan directory should already exist from getNewBuild, but if not, create it
      const existingTestPlan = testsDir.find(path.basename(testPlanBuildDirectory));
      if (!existingTestPlan.record) {
        testsDir.add(path.basename(testPlanBuildDirectory), { entries: [] });
      }
    }

    newTestPlan = newBuild.find(testPlanPath);
    if (!newTestPlan.record) {
      log.error(`Failed to create test plan directory at ${testPlanPath}`);
      return;
    }
  }
  ...
  if (!examplePathOriginal) {
    const testPlanPath = subfolder ? `tests/${subfolder}/${testPlanName}` : `tests/${testPlanName}`;
    log.error(`ERROR: Valid 'reference' value not found in "${testPlanPath}/data/references.csv".`);
  }
  const exampleRecord = testPlanRecord.find(examplePathOriginal);
  if (!exampleRecord.isFile()) {
    const testPlanPath = subfolder ? `tests/${subfolder}/${testPlanName}` : `tests/${testPlanName}`;
    log.error(
      `ERROR: Invalid 'reference' value path "${examplePathOriginal}" found in "${testPlanPath}/data/references.csv".`
    );
  }
