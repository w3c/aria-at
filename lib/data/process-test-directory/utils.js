...
class Utils {
  ...
  getNewBuild(testPlanUpdate) {
    ...
    if (this._subfolder) {
      testsFolderEntries.push({
        name: this._subfolder,
        entries: [testPlanEntry],
      });
    } else {
      testsFolderEntries.push(testPlanEntry);
    }

    testsFolderEntries.push({ name: 'support.json', ...this._supportJsonRecord.record });
  ...
  // HTML Builders
  getIndexHtml({ atHeaders, rows }) {
    const headers =
      this.testFormatVersion === 1
        ? `<th>Task ID</th>
        <th>Testing Task</th>
        ${atHeaders}
...
`;
  ...
  getTestHtml({
    ...
    commandsJson,
    exampleReferences,
    testPageAndInstructionsPath,
  }) {
    let filesPromiseJs;
    if (this.testFormatVersion === 1) {
      filesPromiseJs = `new Promise((resolve) => {
    fetch('../../support.json')
      .then(response => resolve(response.json()))
    })
  .then(supportJson => {
    const testJson = ${testJson};
    const commandJson = ${commandsJson};
    initialize(supportJson, commandJson);
    verifyATBehavior(testJson);
    displayTestPageAndInstructions(${testPageAndInstructionsPath});
  });`;
    } else {
      filesPromiseJs = `new Promise((resolve) => {
    fetch('../../support.json')
 ...
 * @param {string} testPlanName - Name of the test plan
 * @returns {object} Object containing subfolder and subfolderName
 */
function detectSubfolderStructure(testsDirectory, testPlanName) {
  // First check if test plan exists directly in tests directory
  const directPath = path.join(testsDirectory, testPlanName);
  if (fs.existsSync(directPath)) {
    return { subfolder: null, subfolderName: null };
  }

  // Look for test plan in subfolders
  try {
    const subfolders = fs
    ...
    for (const subfolder of subfolders) {
      const subfolderPath = path.join(testsDirectory, subfolder, testPlanName);
      if (fs.existsSync(subfolderPath)) {
        // Try to get subfolder name from support.json
        let subfolderName = subfolder; // Default to folder name
        try {
          const supportJsonPath = path.join(testsDirectory, 'support.json');
          if (fs.existsSync(supportJsonPath)) {
            const supportJson = JSON.parse(fs.readFileSync(supportJsonPath, 'utf8'));
            if (supportJson.subfolders && supportJson.subfolders[subfolder]) {
              subfolderName = supportJson.subfolders[subfolder];
            }
          }
        } catch (error) {
          // If we can't read support.json, just use the folder name
          this.log.error('error.read.support.json', error);
        }

        return { subfolder, subfolderName };
      }
    }
  } catch (error) {
    // If we can't read the directory, assume no subfolder
    this.log.error('error.read.tests.no.subfolder', error);
  }
