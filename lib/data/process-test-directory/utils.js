...
class Utils {
  ...
  addCommandError(
    ...
    if (this.testFormatVersion === 1) {
      this.errors += `[Command]: The key reference "${key}" found in "tests/${testPlanName}/data/commands.csv" for "test id ${testId}: ${task}" is invalid. Command may not be defined in "tests/resources/keys.mjs".\n`;
    } else {
      this.errors += `[Command]: The key reference "${key}" found in "tests/${testPlanName}/data/${atKey}-commands.csv" for "test id ${testId}" is invalid. Command may not be defined in "tests/commands.json".\n`;
    }
  }
  ...
  addCommandAssertionExceptionError(
    ...
    this.errorCount += 1;
    this.errors += `[Command]: assertionExceptions reference "${assertion}" found in "tests/${testPlanName}/data/${atKey}-commands.csv" for "test id ${testId}" is invalid.\n`;
  }
