...
async function createManifest(manifest, fn) {
  if (typeof fn === 'object') {
    ...
    return _createManifest(manifest, [], [], (submanifest, pathComponents, parents) => {
 ...
 * @returns {Promise<DiskManifest>}
 */
async function readdir(root, manifest) {
  try {
 ...
 * @returns {Promise<DiskManifest>}
 */
async function readFile(root, manifest) {
  try {
 ...
 * @returns {Promise<DiskManifest>}
 */
export async function readDeep(root, manifest) {
...
export async function readShallow(root, manifest) {
  if (!manifest || (manifest.entries === undefined && manifest.buffer === undefined)) {
    try {
      ...
      if (error.code === 'ENOTDIR') {
        const buffer = await fs.promises.readFile(root);
        return { ...manifest, buffer };
      }
