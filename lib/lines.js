...
function reindent(templateArray, ...args) {
  return Lines.cast(templateArray.map(Lines.cast).reduce((carry, subtemplate, index) => carry.indent(args[index - 1]).concat(subtemplate)))
}
...
class Lines {
  ...
  static cast(lines) {
    if (lines instanceof Lines) {
      ...
      return new Lines(String(lines).split('\n').map(line => new Line(line)));
    }
  ...
  indent(linesRaw) {
    const lines = Lines.cast(linesRaw);
    if (this.lines.length === 0) {return lines;}
    return new Lines([
      ...sliceButLast(this.lines),
