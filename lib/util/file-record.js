...
function createHost({ fs = _fs.promises, path = _path } = {}) {
  ...
  async function read(root, { glob = '**' } = {}) {
    const matchesGlob = compileSimpleGlob(glob);
    return walk(record, (record, splitPath) => {
      const subpath = splitPath.join(path.sep);
      if (matchesGlob(subpath)) {
        return { ...record, ...readFS(path.join(root, subpath)) };
      }
      return record;
    });
  }
  ...
  function insertInRecord(record, filepath, item) {
    ...
    if (splitPath.length > 1) {
      ...
      insertInRecord(entry, splitPath.slice(1), buffer);
    } else {
      record.entries.push({ name: splitPath[0], ...item });
    }
  }
  ...
  function findInRecord(record, subpath) {
    ...
    if (typeof subpath === 'string') {
      ...
      splitPath = subpath.split(path.sep);
  ...
  function _filterRecords(record, splitPath, test) {
    if (isDirectory(record)) {
      const recordPath = record.name ? [...splitPath, record.name] : splitPath;
      return {
        ...record,
        entries: record.entries
          .filter(subrecord => test(subrecord, recordPath))
          .map(subrecord => _filterRecords(subrecord, recordPath, test)),
      };
    }
    return record;
  }
  ...
  function filterRecords(record, test) {
    if (test.glob) {
      const globTest = compileSimpleGlob(test.glob);
    ...
    return _filterRecords(record, [], test);
  }
