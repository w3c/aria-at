...
function reindent(templateArray, ...args) {
  return Lines.cast(
    templateArray
      .map(Lines.cast)
      .reduce((carry, subtemplate, index) => carry.indent(args[index - 1]).concat(subtemplate))
  );
}
...
class Line {
  ...
  indent(lines) {
    ...
    return [new Line(this, lines.lines[0]), ...lines.lines.slice(1).map(l => new Line(indent, l))];
  }
...
class Lines {
  ...
  /**
   * @param {string | Lines | {toString: function(): string}} lines
   * @returns {Lines}
   */
  static cast(lines) {
  ...
  /**
   * @param {string | Lines | {toString: function(): string}} linesRaw
   * @returns {Lines}
   */
  concat(linesRaw) {
    ...
    return new Lines([...sliceButLast(this.lines), ...last(this.lines).concat(lines)]);
  }
  ...
  indent(linesRaw) {
    ...
    return new Lines([...sliceButLast(this.lines), ...last(this.lines).indent(lines)]);
  }
