...
const createExampleTests = function (directory) {
  ...
  const support = JSON.parse(
    fse.readFileSync(path.join(rootDirectory, 'tests', 'support.json'))
  );
  let allATKeys = [];
  let allATNames = [];
  ...
  try {
    ...
    console.log(
      "The test directory '" +
        testDirectory +
        "' does not exist. Check the path to tests."
    );
    process.exit();
  }
  ...
  try {
    ...
    console.log(
      "The tests.csv file does not exist. Please create '" +
        testsFile +
        "' file."
    );
    process.exit();
  ...
  try {
    ...
    console.log(
      "The at-commands.csv file does not exist. Please create '" +
        atCommandsFile +
        "' file."
    );
    process.exit();
  ...
  function createTestFile(test, refs, commands) {
    ...
    let testFileName =
      ...
      id +
      '-' +
      cleanTask(test.task).replace(/\s+/g, '-') +
      '-' +
            ...
            .on('end', () => {
              ...
              if (errorCount) {
                console.log(
                  '\n\n*** ' +
                    errorCount +
                    ' Errors in tests and/or commands ***'
                );
                console.log(errors);
              } else {
