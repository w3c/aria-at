...
const support = JSON.parse(
  fse.readFileSync(path.join(testDir, 'support.json'))
);
let allATKeys = [];
...
fse.readdirSync(testDir).forEach(function (subDir) {
  ...
  if (stat.isDirectory() && subDir !== 'resources') {
    ...
    const referencesCsv = fs.readFileSync(
      path.join(subDirFullPath, 'data', 'references.csv'),
      'UTF-8'
    );
    ...
    fse.readdirSync(scriptsPath).forEach(function (scriptFile) {
      ...
      scripts.push(
        `\t${
          scriptFile.split('.js')[0]
        }: function(testPageDocument){\n${script}}`
      );
    });
    ...
    fse.readdirSync(subDirFullPath).forEach(function (test) {
      if (
        ...
        const root = np.parse(fse.readFileSync(testFile, 'utf8'), {
          script: true,
        });
        ...
        let testData = JSON.parse(
          fse.readFileSync(
            path.join(subDirFullPath, path.parse(test).name + '.json'),
            'utf8'
          )
        );
        ...
        const mode =
          typeof testData.mode === 'string' ? testData.mode : testData.mode[0];
        ...
        for (const atKey of allReleventATs.map((a) => a.toLowerCase())) {
          ...
          if (
            testData.additional_assertions &&
            testData.additional_assertions[at.key]
          ) {
        ...
        const lastEdited = output.stdout
          .toString()
          .replace(/"/gi, '')
          .replace('\n', '');
